...
## Module inventory (36 public modules)
...
| Shape queries | `envelope`, `expand`, `centroid`, `convex_hull`, `azimuth`, `num_geometries`, `num_interior_rings`, `num_points`, `num_segments` |
| Mutation / transform | `correct`/`correct_closure`, `reverse`, `unique`, `remove_spikes`, `simplify`, `densify`, `transform`, `line_interpolate` |
| Traversal | `for_each` |
...
`distance`/`distance_with`/`comparable_distance`/`comparable_distance_with`,
...
`envelope`, `expand`/`expand_with`, `centroid`/`centroid_with`, `convex_hull`, `azimuth`/`azimuth_with`,
`correct`/`correct_closure`, `reverse`, `unique`, `remove_spikes`, `simplify`, `densify`,
